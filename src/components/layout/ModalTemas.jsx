...
import { X, Moon, Sun, Palette, Download, Upload, Sparkles, Gem, Waves, Sunset, Leaf, Clock } from 'lucide-react';
...
const ModalTemas = ({ onClose }) => {
    ...
    // Estado para la frecuencia del recordatorio
    const [reminderFreq, setReminderFreq] = useState(
        localStorage.getItem('backup_reminder_frequency') || '1_week'
    );

    const handleFreqChange = (freq) => {
        setReminderFreq(freq);
        localStorage.setItem('backup_reminder_frequency', freq);
    };
            ...
            >
                ...
                <div className="space-y-6">
                    ...
                    <div>
                        ...
                        <div className={`mb-4 p-3 rounded-xl ${modoOscuro ? 'bg-slate-800/50' : 'bg-slate-50'}`}>
                            <div className="flex items-center gap-2 mb-2">
                                <Clock size={14} className={modoOscuro ? 'text-slate-400' : 'text-slate-500'} />
                                <span className={`text-xs font-medium ${modoOscuro ? 'text-slate-300' : 'text-slate-600'}`}>
                                    Recordatorio de Backup
                                </span>
                            </div>
                            <div className="grid grid-cols-4 gap-2">
                                {[
                                    { value: '1_week', label: '1 Sem' },
                                    { value: '2_weeks', label: '2 Sem' },
                                    ...
                                    <button
                                        ...
                                        className={`py-1.5 px-2 rounded-lg text-xs font-medium transition-all ${reminderFreq === opt.value
